<?php
/**
 * AIvance — 도입 문의 폼 엔드포인트
 *
 *  GET  /contact.php?action=token   → CSRF 토큰 발급 (세션 저장 + JSON 반환)
 *  POST /contact.php                → 문의를 SQLite 에 저장하고 Slack 으로 알림
 *
 * 이 엔드포인트는 메일을 발송하지 않는다. 폼이 외부 주소로 메일을 내보내는 경로가
 * 존재하는 한 스팸 발송 릴레이로 악용될 수 있으므로, 접수는 저장 + Slack 알림까지만
 * 하고 실제 메일 발송은 로그인이 필요한 admin.php 에서 사람이 직접 한다.
 *
 * 자격증명은 .env 에서만 로드한다 (하드코딩 금지).
 */
...
require __DIR__ . '/lib/slack.php';

/* 스택 트레이스 노출 금지 — 예외는 JSON 500 으로 변환하고 파일 로그에만 기록 */
ini_set('display_errors', '0');
set_exception_handler(static function (\Throwable $e): void {
    app_log('contact-fatal', $e->getMessage() . ' @ ' . $e->getFile() . ':' . $e->getLine());
    if (!headers_sent()) {
...
/** 접수 성공 응답 (실제 접수와 스팸 조용한 처리 모두 같은 응답을 쓴다). */
function respond_accepted(): never
{
    respond(200, ['ok' => true, 'message' => '문의가 정상 접수되었습니다. 영업일 기준 1일 내 회신드리겠습니다.']);
}

/**
 * DB 저장을 시도하고, 실패하면 원문을 파일에 남긴 뒤 0 을 돌려준다.
 *
 * 메일 발송을 없앤 뒤로 SQLite 가 문의의 유일한 보관처다. 디스크/권한 문제로 INSERT 가
 * 실패했다고 문의 자체를 흘려보내면 복구할 방법이 없으므로, 최소한 원문은 반드시 남긴다.
 *
 * @param array<string, string> $fields
 */
function store_inquiry(array $fields, InquiryStatus $status, string $spamReason = ''): int
{
    try {
        return inquiry_insert($fields, $status, $spamReason);
    } catch (\Throwable $e) {
        app_log('inquiry-fallback', json_encode([
            'error'  => $e->getMessage(),
            'status' => $status->value,
            'ip'     => client_ip(),
            'fields' => $fields,
...
if ($clientIp === '' || !rl_allow('contact:' . $clientIp, 60, 5)) {
...
if (!rl_allow('contact:__global__', 3, 30)) {
...
/* ── 내용 스팸 필터 — URL 2개 이상 또는 스팸 키워드 포함 ─────────────────── */
...
/* 봇 게이트를 모두 통과했지만 내용/도메인이 스팸으로 보이는 제출은 버리지 않고
 * status=spam 으로 저장한다. 오탐이면 관리자 페이지에서 되살릴 수 있고,
 * Slack 알림은 보내지 않아 실제 문의만 알림으로 남는다. */
$spamReason = '';
if (in_array($emailDomain, $disposableDomains, true)) {
    $spamReason = 'disposable_email:' . $emailDomain;
} elseif (looks_spammy($name . ' ' . $message)) {
    $spamReason = 'content_filter';
}

if ($spamReason !== '') {
    log_spam_block('stored_as_spam', ['reason' => $spamReason]);
    store_inquiry($fields, InquiryStatus::Spam, $spamReason);
    respond_accepted();
}

/* ── 저장 → Slack 알림 ────────────────────────────────────────────────── */
$inquiryId = store_inquiry($fields, InquiryStatus::New);

/* DB 저장에 실패해도(원문은 inquiry-fallback 로그에 남아 있다) 알림은 반드시 보낸다 —
 * 오히려 이때가 사람이 즉시 알아야 하는 상황이다. */
if ($inquiryId === 0) {
    app_log('inquiry-fallback', 'DB 저장 실패 — writable/logs/inquiry-fallback-*.log 의 원문을 수동 복구할 것');
}

/* Slack 실패는 접수 실패가 아니다 — 원본은 이미 보관돼 있으므로 알림만 포기하고 로그를 남긴다. */
slack_notify_inquiry($inquiryId, [
    '이름 / 회사' => $name,
    '이메일'      => $email,
    '연락처'      => $phone,
    '관심 제품'   => $product,
